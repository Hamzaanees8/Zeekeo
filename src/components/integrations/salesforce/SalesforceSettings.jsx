...
import { updateSalesforceSettings } from "../../../services/integrations";
...
const SalesforceSettings = () => {
  ...
  useEffect(() => {
    if (user?.integrations?.salesforce?.settings) {
      setSettings(user.integrations.salesforce.settings);
    }
  }, [user]);
  ...
  const handleSave = async () => {
    try {
      await updateSalesforceSettings({ settings });
      ...
      toast.error("Failed to update Salesforce settings.");
    }
  };
