...
import {
  createSalesforceField,
  getSalesforceFields,
} from "../../../services/integrations";

const SalesforceCustomField = ({ onClose }) => {
  ...
  useEffect(() => {
    const fetchFields = async () => {
      try {
        const fields = await getSalesforceFields();
        setCustomFieldNames(fields);
      } catch (err) {
        console.error("Failed to fetch Salesforce fields", err);
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await createSalesforceField({
  ...
  return (
    ...
    >
      <div className="bg-white w-[570px] px-7 pt-[15px] pb-[28px] rounded-[8px]  border border-[#D6D6D6] bg-white rounded-[8px] shadow-sm">
        ...
        <p className="text-[#7E7E7E] mb-[21px] font-[500] font-urbanist text-[16px]">
          Integration with Salesforce requires a contact field for storing
...
export default SalesforceCustomField;
