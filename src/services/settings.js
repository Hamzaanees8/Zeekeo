...
export const updateUsers = async (data) => {
  const { auto_scale, time_zone, auto_adjust, schedule, limits, first_name, last_name, company, email, phone_number, password } = data;
  ...
  const transformedLimits = limits ? {
    ...
    linkedin_view: limits.find(l => l.label === "Profile Views")?.value || 100,
    linkedin_invite: limits.find(l => l.label === "Invites")?.value || 100,
    linkedin_inmail: limits.find(l => l.label === "InMails")?.value || 100,
    linkedin_message: limits.find(l => l.label === "Sequence Messages")?.value || 100,
    linkedin_follow: limits.find(l => l.label === "Follows")?.value || 100,
    linkedin_like_post: limits.find(l => l.label === "Post Likes")?.value || 100,
    linkedin_endorse: limits.find(l => l.label === "Endorsements")?.value || 100,
    email_message: limits.find(l => l.label === "Email Sequence Messages")?.value || 100,
  } : undefined;
