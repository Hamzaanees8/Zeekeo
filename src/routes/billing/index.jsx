...
import { BlueAdd, Tooltip } from "../../components/Icons";
import Subscription from "./components/Subscription";
import OtherSubscription from "./components/OtherSubscription";
import InvoiceModal from "./components/InvoiceModal";
import { GetPlans, GetSavedCards } from "../../services/billings";
...
const Billing = () => {
  ...
  const handleSubmit = () => {
    ...
    if (Object.keys(errors).length === 0) {
      // You can send the data to API or log it here
  ...
  return (
    <SubscriptionProvider>
      <div className="flex bg-white w-full">
        <SideBar />
        <div className="w-full flex flex-col justify-start items-start gap-y-[26px] pl-[40px] pr-4 py-[62px] bg-[#EFEFEF]">
          <h1 className="font-medium text-[#6D6D6D] text-[48px] font-urbanist">
            Billing
          </h1>
          <div className="w-full flex flex-col text-[#6D6D6D] gap-y-2">
            <p className="font-urbanist font-medium text-[20px]">Invoices</p>
            <Table
              setShowInvoice={setShowInvoice}
              setSelectedInvoiceUrl={setSelectedInvoiceUrl}
            />
          </div>
          <div className="w-full flex justify-start items-start mt-[14px] gap-x-[64px]">
            <div className="w-full flex flex-col gap-y-7 text-[#6D6D6D]">
              <p className="text-[20px] font-medium">Cards</p>
              <div className="flex flex-col gap-y-2.5">
                {cards.map(card => (
                  <label
                    key={card.id}
                    className="flex items-center gap-x-4 cursor-pointer text-[#6D6D6D] text-[16px] font-normal"
                  >
                    <input
                      type="checkbox"
                      name="savedCard"
                        ...
                        <div className="absolute top-1/2 right-3 -translate-y-1/2">
                          <div className="relative inline-block">
                            ...
                            <div className="absolute bottom-full right-0 mb-1 w-56 bg-gray-800 text-white text-xs rounded px-2 py-1 opacity-0 peer-hover:opacity-100 transition-opacity duration-200 z-10 whitespace-normal pointer-events-none">
                              Enter your official VAT number issued by your
                              local tax authority.
                            </div>
                          </div>
                        </div>
