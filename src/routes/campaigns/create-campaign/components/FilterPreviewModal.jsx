    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center border border-[#7E7E7E]">
      <div className="bg-white w-full max-w-4xl shadow-lg p-6 relative">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-[#04479C] text-lg">
            Filter Preview
          </h2>
          <button onClick={onClose} className="text-xl  text-gray-600 cursor-pointer ">
            &times;
          </button>
        </div>

        <p className="flex text-[15px] text-[#6D6D6D] mb-4 gap-3 items-center"><FilterProfile />4 Profiles</p>
        ...
        <table className="w-full text-sm border-t border-[#7E7E7E]">
          ...
          <tbody>
            {mockProfiles.map((profile, idx) => (
              <tr key={idx} className="border-b border-[#7E7E7E] text-[#6D6D6D] hover:bg-[#f9f9f9] pt-1">
                <td className="py-2 px-2">{idx + 1}</td>
                <td className="py-2 px-2 flex"><span className="w-[35px] h-[35px] rounded-full bg-[#D9D9D9]"></span></td>
        ...
        <div className="flex justify-between mt-6">
          <button className="bg-[#7E7E7E] text-white px-6 py-2 text-sm cursor-pointer" onClick={onClose}>
            Back
          </button>
          <button className="bg-[#0387FF] text-white px-6 py-2 text-sm cursor-pointer" onClick={onCreate}>
            Create Filter
          </button>
        </div>
      </div>
