...
import { createTemplate, deleteTemplate, getTemplates, updateTemplate } from "../../../../services/templates.js";
import { groupTemplatesByType, insertTextAtCursor, variableOptions } from "../../../../utils/template-helpers.js";
import {
  PlusIcon,
  PencilIcon,
  CopyIcon,
...
const SavedMessages = ({ selectedTemplateId = null, type = "linkedin_message", onAssignTemplate }) => {
  ...
  const handleVariableInsert = (variable, fieldRef, fieldVal, mode = 'new') => {
  ...
  return (
    <div className="space-y-2">
      ...
      {type && <div className="pt-2 text-[18px] text-[#454545] py-2 font-medium border-b border-b-[#7E7E7E]"><span>Templates</span></div>}
      {type && groupedTemplates[type]?.length > 0 ? (
        groupedTemplates[type].map((template, idx) => (
          <div key={idx} className="m-0">
            ...
            <div className="flex items-center justify-between py-2 ">
              <span
                className={`font-urbanist text-[16px] cursor-pointer ${template.template_id === selectedTemplateId
                    ? 'text-[#0387FF]'
                    : 'text-[#6D6D6D]'
                  }`}
            ...
            {openIndex === template.template_id && (
              <div className="space-y-4 p-3">
                {template?.subject && <div className="w-full border border-[#C7C7C7] bg-white p-2 text-[#6D6D6D] min-h-[30px]">{template.subject}</div>}
                <div className="w-full border border-[#C7C7C7] bg-white p-2 text-[#6D6D6D] min-h-[100px]">{template.body}</div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {variableOptions.map((opt) => (
                      <button
                        ...
                        onClick={() => handleVariableInsert(opt.value, editTemplateBodyRef, editingTemplate.body, 'edit')}
                      >
                        {opt.label}
                      </button>
                    ))}
        ...
        <div className="text-[#6D6D6D]"><p>No templates available.</p></div>
      )}

      {type && newTemplate &&
        <div className="space-y-4">
          <div className="pt-2 text-[16px] text-[#454545] py-2 font-medium border-b border-b-[#7E7E7E]"><span>Add Template</span></div>
          <input
            type="text"
            placeholder="Title"
            ...
            <div className="flex flex-wrap gap-2">
              {variableOptions.map((opt) => (
                <button
                  key={opt.value}
                  className="text-[16px] text-[#6D6D6D] border border-[#7E7E7E] bg-white px-3 "
                  onClick={() => handleVariableInsert(opt.value, addTemplateBodyRef, newTemplate.body)}
                >
                  {opt.label}
                </button>
              ))}
          ...
          <div className="flex justify-end">
            <button className="px-4 py-1 bg-[#0387FF] text-white" onClick={() => handleSaveTemplate(newTemplate)}>
              Create & Assign
            </button>
          </div>
        </div>
